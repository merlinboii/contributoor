...
export const AppBar: FC = () => {
  ...
  const [mode, setMode] = useState<'Project' | 'Contributor'>();

  // Retrieve mode from local storage on component mount
  useEffect(() => {
    const savedMode = localStorage.getItem('mode');
    if (savedMode === 'Contributor' || savedMode === 'Project') {
      setMode(savedMode as 'Project' | 'Contributor');
    }
  }, []);

// Function to toggle mode
  const toggleMode = () => {
    const newMode = mode === 'Project' ? 'Contributor' : 'Project';
    setMode(newMode);
    localStorage.setItem('mode', newMode); // Save mode to local storage

    if (newMode === 'Project') {
      router.push('/project-dashboard');
    } else if (newMode === 'Contributor') {
      ...
      <div className="navbar flex h-20 flex-row md:mb-2 shadow-lg bg-black text-neutral-content border-b border-zinc-600 bg-opacity-66">
        <div className="navbar-start align-items-center">
          <div className="hidden sm:inline w-22 h-22 md:p-2 ml-10">
            <Link href="/" passHref>
              <span className="text-secondary hover:text-white font-bold text-2xl text-transparent bg-clip-text bg-gradient-to-br from-indigo-500 to-fuchsia-500">Contributoor</span>
            </Link>
          </div>
        ...
        <div className="navbar-end ">
          ...
          <label
            ...
            <div className="HAMBURGER-ICON space-y-2.5 ml-5">
              <div className={`h-0.5 w-6 bg-purple-600 ${isNavOpen ? 'hidden' : ''}`} />
              <div className={`h-0.5 w-6 bg-purple-600 ${isNavOpen ? 'hidden' : ''}`} />
              <div className={`h-0.5 w-6 bg-purple-600 ${isNavOpen ? 'hidden' : ''}`} />
            </div>
